...
export default function FoldText({
    ...
    const renderSegment = (content: string, key: string, split = splitBy) => {
      ...
      const isHighlighted =
        ...
        highlightWords.some((hw) =>
          content.toLowerCase().includes(hw.toLowerCase())
        );
        ...
        >
          <span
            className={`fold-text-piece ${
              isHighlighted ? "italic font-semibold" : ""
            }`}
  ...
  }, [text, splitBy, hinge, hingeConfig.origin, safePerspective, highlightWords, highlightColor]);
